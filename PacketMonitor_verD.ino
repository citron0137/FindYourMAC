...
int rssiPeak = -255;
char rssiPeackMac[18] = "00:00:00-00:00:00";
...
void wifi_promiscuous(void* buf, wifi_promiscuous_pkt_type_t type) {
  ...
  if (type == WIFI_PKT_MGMT && pkt->payload[0] == 0x40) {
    ...
    Serial.print("MANAGEMENT PACKET (");
    /*
    Serial.print("FC :");
    Serial.printf("0x%02X", pkt->payload[0]);
    Serial.printf("0x%02X", pkt->payload[1]);
    Serial.print("|");
    */
    Serial.print("rssi :");
    Serial.printf("%d", (int)ctrl.rssi);
    Serial.print("|");
    Serial.print("mac : ");
    for(int i=0;i<5;i++) {
      Serial.printf("%02X:",pkt->payload[10+i]);
    }
    Serial.printf("%02X",pkt->payload[10+5]);
    Serial.print(")");
    Serial.println();
    if(ctrl.rssi > rssiPeak){
      rssiPeak = ctrl.rssi;
      sprintf(rssiPeackMac,"%02X:%02X:%02X-%02X:%02X:%02X",pkt->payload[10],pkt->payload[11],pkt->payload[12],pkt->payload[13],pkt->payload[14],pkt->payload[15]);
    }
  }
...
void draw() {
  ...
  display.setTextAlignment(TEXT_ALIGN_LEFT);
  display.drawString( 10, 20, ("PEAK RSSI : "));
  display.drawString( 10, 40, ("PEAK MAC : "));
  display.drawString( 20, 50, (rssiPeackMac));
  //display.drawString( 20, 50, ("00:00:00"));
  display.setTextAlignment(TEXT_ALIGN_RIGHT);
  display.drawString( 100, 30, (String)rssiPeak);
...
void setup() {
  ...
  display.drawString(6, 6, "FindYourMAC");
  display.setFont(ArialMT_Plain_10);
  display.drawString(24, 34, "Made by");
  display.drawString(29, 44, "@Citron0137");
  display.display();
